...
def fill_movie_details(movie):
  ...
  if movie.is_full_record:
    logger.warning(
      f"{movie.title} is already a full record."
    )
    return
...
def search_and_save(search):
  ...
  earch result to the local DB as a partial record.
  """
  ...
  if not created and (search_term.last_search > now() - timedelta(days=1)):
    ...
    logger.warning(
      f"Search for '{normalized_search_term}' was performed in the past 24 hours so not searching again."
    )
    return
